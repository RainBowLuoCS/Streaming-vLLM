...
class BlockManager:
    ...
    def can_append_slots(self, seq, extra_slots=0):
        """Ensure blocks exist through position len(seq) + extra_slots - 1."""
        target_tokens = len(seq) + extra_slots
    ...
    def may_append_slots(self, seq, extra_slots=0):
        target_tokens = len(seq) + extra_slots
        target_blocks = (target_tokens + self.block_size - 1) // self.block_size
