...
def pure_move_kv_kernel(
    ...
    """Pure memory copy. NO RoPE update."""
    pid = tl.program_id(0)
...
def inplace_delta_rope_kernel(
    k_cache_ptr,
    cos_ptr, sin_ptr,
    slots_ptr, deltas_ptr,
    num_ops,
    num_kv_heads: tl.constexpr, head_dim: tl.constexpr,
    half_dim: tl.constexpr, pos_offset: tl.constexpr,
):
    """In-place Delta RoPE using exact pre-calculated delta."""
    pid = tl.program_id(0)
    if pid >= num_ops: return
...
def build_rope_cache(head_dim, max_pos, rope_theta, dtype=torch.float16):
    half = head_dim // 2
